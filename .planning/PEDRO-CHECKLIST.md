...
- **`db:migrate`** — migration `0057_sour_peter_quill` (adds `subscriptions.last_event_at`) must run
  before billing is enabled, or the webhook's ordered-sync path errors. `npm run db:migrate` (via the
  sanctioned `deploy-migrate-prod.yml` path once its `environment: production` scoping is fixed — see ledger).
...
## 5. Two dark-seam follow-ups (buildable by me next, not blocking)
Both shipped as live-but-partial; noted here so they don't get lost:
