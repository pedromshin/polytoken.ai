...
- [ ] **LIVE-06**: W-1 closed — the screenshot harness SURFACES covers /emails/[id] and captures authenticated surfaces once a seeded session exists
...
- [ ] **RSKN-06**: The off-token chip/badge stragglers consume the purpose-built tokens (backlog 999.16): inbox entity chips onto `color.graph.entity` + `radius.pill`, `/entities/[id]` StatusBadge onto the `color.tier.*` ladder — explicit scope extension to `/entities/[id]`
