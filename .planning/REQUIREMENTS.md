...
- [x] **CONF-02**: Submitting a confirm-action re-checks the referenced `knowledge_node_edges` row's tier against the declaration snapshot (409 `stale` if promoted/rejected out-of-band) before dispatching through an explicit 2-entry use-case table (`knowledge_edge_tier_promotion`, `entity_merge_confirm`), idempotent by id
