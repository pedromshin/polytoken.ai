...
- ✅ **vNEXT — The Living Canvas** (Phases 73–77) — **SHIPPED/CLOSED 2026-08-08** (code-complete 2026-08-06; CLOSED 2026-08-08 with all seven live-acceptance seams resolved **ACCEPT-AS-DEBT, not EXECUTED** — the code is done and unit-proven, the behaviour is UNWITNESSED on prod; owner Pedro, review by 2026-08-22, listed in [PEDRO-CHECKLIST.md](PEDRO-CHECKLIST.md) §0b and the ledger in [milestones/vNEXT-AUDIT-2026-08-06.md](milestones/vNEXT-AUDIT-2026-08-06.md)) (all five phases built + adversarially verified on `main`; remaining = human-gated LIVE seams only: migration 0061 on prod, worker provisioning, flag flips `CANVAS_EMIT_TOOL_ENABLED`/`MORNING_BOARD_ENABLED`/`CASCADE_CORRECTION_ENABLED`/`RECIPE_RECOMPUTE_ENABLED`/`INGEST_ENQUEUE_ENABLED`, live UAT LCAN-05/LCAN-09 · MORN-07 · BTAP-07 · MCPX-09 · CPF-live, and the real-browser pass — see the SHIPPED-STATUS block in [milestones/vNEXT-living-canvas-ROADMAP.md](milestones/vNEXT-living-canvas-ROADMAP.md); shipped as an orchestrator run, without per-plan GSD VERIFICATION.md trails). The five "bangers": the canvas stops being a whiteboard the agent draws *on* and becomes a living substrate the agent *builds* — agent-authored live dataflow recipes that recompute and survive reload (73, the trunk), a morning board that assembles itself headless overnight (74), a correct-once/propagate-everywhere flywheel over the human-gated trust ladder (75), bespoke disposable code-island apps wired to your real rows inside the opaque-origin jail (76), and your compounding personal graph projected as expose-only MCP tools your own Claude Code can call (77). The capability spine (v1.11) cashing in: canvas verbs, read procedures, and the jailed generator all become things the agent *drives* and external agents *call*, because each was declared once, ownership-gated once, tested once. Phase 73 is XL and the foundation (coins the `shared.published.*` publish port + the `canvas_recipes` row); 74–77 are four independent L branches off it. Depends on Phase 66 (durable graphile-worker + Canvas/Node/Edge rows) and the just-shipped 11 canvas node types + `canvas.addNode` agent-draws-node wedge (already one third of 73's loop). 41 requirements (LCAN 9 + MORN 7 + CPF 6 + BTAP 10 + MCPX 9). Roadmap: [milestones/vNEXT-living-canvas-ROADMAP.md](milestones/vNEXT-living-canvas-ROADMAP.md).
- 🚧 **vLAUNCH — Durable Mail & First Dollar** (Phases 78–81) — **CURRENT, opened 2026-08-07** (blessed per Pedro's marching orders; formalized under the nonstop order — assumption A1 in [ASSUMPTIONS-2026-08-07.md](ASSUMPTIONS-2026-08-07.md)). Deliberately operations-not-engineering: 78 Durable Ingest Cutover (CUT-01..10, the graphile-worker runbook end-to-end, staging rehearsal before prod), 79 Billing Go-Live (BILL-01..07 — billing already LIVE since 2026-08-06; 🚦BILL-05 legal/MoR gates public pricing), 80 Live-Acceptance Burn-Down (BURN-01..06 — executes the vNEXT audit's 7-seam menu, then closes vNEXT), 81 Wedge Opener (WEDG-01..04 — cascade live on real mail + the learning-loop baseline, LAST and SERIAL). 27 requirements. Wave 0 prep SHIPPED 2026-08-07 (`8263578c`). Roadmap: [milestones/vLAUNCH-ROADMAP.md](milestones/vLAUNCH-ROADMAP.md) · Proposal: [milestones/PROPOSAL-vLAUNCH.md](milestones/PROPOSAL-vLAUNCH.md) · Execution law: [milestones/VLAUNCH-WAVE-PLAN.md](milestones/VLAUNCH-WAVE-PLAN.md).
...
- [x] 55-02-PLAN.md — Stage 2 (crux): port globals.css to oklch + @theme inline + @source, fix every hsl(var(--x)) call site, adapt the genui re-theme surface [STCK-01]
...
- [x] 60-06-PLAN.md — Remaining editor panels + detail frame (serif subject, ink error); role-hue-ban.test.ts (scoped, ratcheting)
- [x] 60-07-PLAN.md — 16-surface screenshot harness capture (criterion 4) or honest gap; brand guide + design-system skill for Phases 61-63
**UI hint**: yes
...
- [x] 61-02-PLAN.md — The tier truth promoted to one shared home; canvas-vocabulary.ts grown from it (tier on edges, kind as geometry); the edge-tier x node-kind matrix gate [SURF-02]
- [x] 61-03-PLAN.md — The chat frame, conversation rail and composer redesigned (one header rule, registry rail, ink send); the frame structure gate [SURF-02]
...
- **999.21 — Pre-existing sidebar pointer-events E2E interception bug (filed 2026-07-15, found during v1.9 §G.2 and re-confirmed by 55-VERIFICATION):** a sidebar element intercepts pointer events in Playwright runs, failing several E2E specs (identical signature reproduced in isolation on the pre-Phase-55 baseline — NOT a migration regression; see `.planning/phases/55-platform-migration-tailwind-v4-react-19/deferred-items.md` and 55-VERIFICATION.md). Also pre-existing: a `packages/genui` `artifacts.test.ts` content-hash-drift failure. Both are candidates for an opportunistic fix during the v1.10 surface-redesign band (Phases 60-62 touch the sidebar and genui surfaces anyway).

- **999.22 — `npm run build:local` silently corrupts a running dev server's `.next`, and our own plans mandate it (root-caused 2026-07-15 during 60-07, HIGH — this one bites every phase):** `next build` and `next dev` share `apps/web/.next` by default, so running the build while a dev server is up overwrites the dev server's server chunks with production output. The dev server keeps its in-memory module graph and starts throwing `Cannot find module './N.js'`; the app then serves SSR HTML whose **client JS never executes** — skeleton rows that never resolve, the theme toggle stuck on its `opacity-0` pre-mount placeholder, inert tab clicks, and `/emails/[id]` rendering Next's runtime-error overlay. **It fails silently:** the build reports success and the damage only surfaces when a human next opens the app. Proven by mtime forensics: production-only artifacts (`.next/BUILD_ID`, `required-server-files.json`, `prerender-manifest.json`, and `server/pages/_document.js` — a *Pages-Router* file in this App-Router app) all written 19:40, sitting beside `.next/static/development`. **The trap is in our own instructions:** `cd apps/web && npm run build:local` is a standing line in the `<verification>` block of every Phase 59-63 plan, so each plan that follows it destroys the running server. This is what blocked ROADMAP criterion 4 in Phase 60 (see `60-SCREENSHOT-REVIEW.md` → "Root cause"). **Fix:** give the build its own `distDir`/`NEXT_DIST_DIR` so it cannot collide, or change the verification line to require the dev server be stopped first and `.next` wiped after. Recovery today: stop the server, `rm -rf apps/web/.next`, restart `npm run web:dev`.

- **999.23 — The screenshot harness has no theme axis and reports `ok` while photographing a crash (filed 2026-07-15 during 60-07):** two independent holes in `apps/web/e2e/screenshot-review.spec.ts`. (a) **Dark mode has never once been captured.** The spec varies exactly `BASE_SURFACES × VIEWPORTS {mobile 390, desktop 1440}` — there is no theme axis — yet `globals.css` ships a full `.dark` block (line 625) whose hue/chroma are deliberately held constant against `:root`, and Phase 59 re-tokened both themes. Every "verified in both themes" claim in the Phase 59-60 planning is therefore unbacked by evidence. A naive "click the Dark mode button" step would silently no-op, because the toggle is hydration-gated exactly like the studio pack switcher; the correct fix is Playwright's `colorScheme: "dark"` context option (next-themes defaults to `system`), which needs no click. (b) **The capture spec asserts nothing** — by design it is a camera, not a gate — so all four 60-07 runs exited `1 passed` while one surface rendered a runtime error and the rest never hydrated. A camera that photographs a crash and reports success is how a corrupted build ships through a green pipeline. Add a cheap liveness assertion: fail if any capture contains Next's error-overlay text, or if a known post-hydration element never appears. Related: 999.22 (the corruption these runs failed to flag), and the phase's own lesson that a green gate is not a look.

- **999.39 — Cloud Desktop panel: spawn a cloud machine from polytoken and operate its realtime remote desktop AS AN IN-APP COMPONENT (raised 2026-07-20, user — MAJOR PRODUCT FEATURE, was not previously captured):** the user's vision, in their own words: *"a remote virtual desktop which we can spawn from within polytoken ui and use it remotely in realtime from within polytoken … our frontend will show a component that renders the realtime remote desktop and we can literally use it like a normal computer, it's just it's remote on the cloud."* This is **the realization of VISION E5** as the user actually means it — not a *browser* panel but a **whole remote computer rendered in-app**, and NOT the D3/999.26 remote desktop (those are RDP/Sunshine into the *user's own physical Windows PC*). The user's intent is stronger: *"i want to have a remote computer so that i don't have a local computer at all anymore"* — the cloud desktop REPLACES owning a local machine; polytoken becomes the computer. Here polytoken itself **provisions a fresh cloud machine (VM/container) on demand** and **streams its live desktop into a polytoken UI surface** (a canvas panel / dedicated surface) that the user drives with mouse+keyboard in realtime, exactly like a local computer. The self-use leverage the user named — *"we just make one for ourselves and clone, run and test the repo from within the project itself … test the browser stuff from within our polytoken ui itself"* — is a **use** of the feature (dogfood: spin a machine, `git clone`, run polytoken on localhost there, exercise the browser/UI inside the rendered desktop), NOT its definition. Scope surfaces to design: (1) **provisioning** — a control-plane capability (registry `Capability`, INV-1, `risk: "irreversible"`/costed) that spawns/tears-down a cloud desktop (candidates: Firecracker/microVM, a container-desktop like Kasm/Selkies-on-a-pod, or a cloud-VM API; a machine = a daemon-protocol "job" per VISION guardrail 2, so E7-style job-shaping already anticipated it); (2) **streaming transport** — WebRTC/Selkies-class low-latency pixels+input into the browser (frontier §B researched the tech for the *personal-box* case — REUSE the transport findings, but the target is a **spawned cloud machine**, not the user's PC); (3) **the render component** — a canvas node / panel that embeds the live desktop (the jailed-iframe discipline from Phase 20 + the genui panel-as-node registry are the reuse seam); (4) **lifecycle/cost/security** — session lifetime, teardown, per-user isolation (INV-8..11 tenancy), and cost ceilings, since a running cloud desktop burns real money (relate to Q5's cost-cap stance). **Relation to the roadmap:** E5 is REDEFINED by this — it was scoped as a CDP *browser* panel; the user means a *whole machine*, cloud-spawned, polytoken owning its lifecycle, replacing the local computer. Sequence as a v2.x epoch (after the v2.0 daemon + the capability registry it rides on); do NOT conflate with D3's travel floor. **Capture correction:** the user stated this in earlier prompts and it was NOT written into the planning until now — logged here as the canonical record; must be folded into VISION.md / the endgame ladder at the next milestone boundary.
