...
| A19 | **`BILLING_ENABLED` FLIPPED TRUE 2026-08-08** on Pedro's explicit "do it" after the concern was recorded in full. Billing is LIVE on polytoken.ai. This was his call to make and he made it deliberately | `vercel env update BILLING_ENABLED production` -> true on `nauta-web`; redeploy triggered | ⚠️ **OWED: the BILL-05 legal pack is still unpublished** — customers can now be charged under terms that do not exist. Highest-priority follow-up in the repo, ahead of the milestone close. Reverse with `'false' \| vercel env update BILLING_ENABLED production --yes` + redeploy |
