...
- **Inbound SNS handler swallows failures — RE-VERIFIED STILL TRUE ON PROD 2026-08-08.** The durable fix is *written* (Track 3a: `INGEST_ENQUEUE_ENABLED` enqueues a pointer job and returns 500 on enqueue failure so SNS retries), but **all three ingest flags are UNSET on the live task def** (`nauta-services-email-listener:4`), so prod runs code defaults: `INGEST_ENQUEUE_ENABLED=False`, `INGEST_BACKGROUND_ENABLED=False`, `INGEST_INLINE_RETRY_ON_FAILURE=False`. Inbound mail still runs inline and a pre-persist failure returns 200 → SNS never retries → **the email is permanently, silently lost.** ⚠️ Do NOT read "prod durable-ingest seam LIVE" as "mail is durable": the plumbing (graphile schema, `enqueue_job`, allowlist, grant, worker) is live, but **production mail does not flow through it.** **🛑 And there is NO WORKER on prod** — the prod task def holds one container (`email-listener`); the graphile worker is a ship-dark **sidecar** gated on `worker_db_url_secret_arn_prod` (`infrastructure/aws/ecs.tf:39`, default `""`), never set; no worker task-def family, no worker service. So flipping `INGEST_ENQUEUE_ENABLED` **first** would queue every email with nothing to drain it — worse than today. Correct order: `INGEST_INLINE_RETRY_ON_FAILURE=true` (safe now) → deploy the worker sidecar → *then* enqueue. See `.planning/PEDRO-CHECKLIST.md` §0c.
