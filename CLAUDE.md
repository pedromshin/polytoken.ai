...
- **Terraform remote state IS live** — S3 backend active in `main.tf`, `terraform state list` returns **68 resources** (verified 2026-08-08). The older "no remote backend, never apply" note was **stale** and is corrected here; it is why applies were treated as forbidden for weeks. Applies are safe **with a gate**: `terraform plan -out=…`, then machine-check the plan JSON against an allowlist of expected addresses and ABORT on any `aws_ses_*` / `aws_sns_*` / `aws_s3_bucket` / `aws_lb*` / `aws_lambda*` / `aws_route53*` churn. That gate ran on all three applies of 2026-08-08; each touched only the ECS task def, the service, and (once) the execution-role policy, and all 5 SES receipt rules stayed intact. **When adding a Secrets Manager secret, apply `aws_iam_role_policy.ecs_execution_secrets` FIRST with `-target`** — a task that cannot read its secret fails to start, and `essential=false` does not cover that.
...
- **Inbound mail is DURABLE as of 2026-08-08 — the old "swallows failures" landmine is CLOSED.** Prod task def `nauta-services-email-listener:7` runs TWO containers (`email-listener` + the `email-worker` graphile-worker sidecar) with `INGEST_ENQUEUE_ENABLED=true` and `INGEST_INLINE_RETRY_ON_FAILURE=true`. Inbound SNS now enqueues a durable pointer job and a failed enqueue returns 500 so SNS retries; the worker owns retries + dead-lettering. Verified before the flip: worker RUNNING and draining a real job end-to-end (`scripts/prod-enqueue-drain-proof.mjs` = PASS). **Ordering law that made it safe, keep it:** deploy + prove the worker FIRST, flip enqueue LAST — enqueue with no worker queues every email with nothing to drain it, which is worse than the inline path. Rollback = set `ingest_enqueue_enabled_prod = false` in tfvars and apply.
