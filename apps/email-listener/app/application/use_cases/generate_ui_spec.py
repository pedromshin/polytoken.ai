...
class GenerateUiSpecUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        # generate() never raises — returns SAFE_FALLBACK_SPEC on total failure.
        spec = await self._generator.generate(
            extraction=extraction,
            registry_version=registry_version,
        )
        log.info("genui_generate_done", spec_type=spec.get("root", {}).get("type"))

        # ── Determine outcome for audit (D-19, T-13-11) ─────────────────────────
        outcome = _determine_outcome(spec)

        latency_ms = int(time.monotonic() * 1000) - start_ms

        # ── Audit (GEN-05, D-19, T-13-10) ───────────────────────────────────────
        ...
        return GenerateUiSpecResult(spec=spec)


def _determine_outcome(spec: dict[str, Any]) -> Literal["ok", "fallback", "escalated"]:
    """Derive the Literal['ok','fallback','escalated'] outcome from the spec dict.

    A spec matching the SAFE_FALLBACK_SPEC shape (root.type='alert' + title starting with
    'Unable to generate') is treated as a fallback outcome.  All other specs are 'ok'.
    """
    root = spec.get("root", {})
    if (
...
def _resolve_model_id() -> str:
    """Return the configured primary GenUI model ID for the audit event.
    ...
    return get_settings().genui_model_id
