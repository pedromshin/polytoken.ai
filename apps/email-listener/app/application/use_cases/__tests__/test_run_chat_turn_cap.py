...
# only the cap-gate-specific doubles remain here.
# ---------------------------------------------------------------------------
...
async def _run_events(use_case: RunChatTurn) -> list[ChatRunEvent]:
    return [
        event async for event in use_case.run(conversation_id=_CONVERSATION_ID, user_text="Hi", model_id=_TEXT_MODEL.id)
    ]
