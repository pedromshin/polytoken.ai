/**
 * apps/web/src/app/api/chat/sources/[ledgerId]/promote/route.ts — server-side-keyed
 * proxy for POST /v1/chat/sources/{ledger_id}/promote (Phase 63 canon-curation,
 * RCNV-01 promotion-gate seam).
 *
 * Copies apps/web/src/app/api/knowledge/edges/[edgeId]/promote/route.ts's
 * structure verbatim: request-time env read (never module-init, never a public
 * NEXT_PUBLIC var), Zod-validated route param + body, a jsonError helper, and
 * meaningful-4xx pass-through with friendly, non-leaking messages.
 * EMAIL_LISTENER_API_KEY is read ONLY here, at request time, and never reaches
 * client-importable code (T-32-06, mirrors T-24-13).
 *
 * Unlike a streaming proxy, this is a plain JSON request/response — on success
 * the upstream's { ledger_id, node_id, status } view is returned as-is
 * (status 200). The canon toolbar (`canon-toolbar.tsx`) only inspects
 * response.ok plus the { error } message on failure.
 */
...
const LEDGER_ID_SCHEMA = z.string().uuid();
...
const REJECTION_MESSAGES: Readonly<Record<number, string>> = {
  404: "This source could not be found.",
  409: "This source can no longer be added to the canon.",
  403: "This source belongs to another workspace.",
};
...
interface RouteParams {
  readonly params: Promise<{ readonly ledgerId: string }>;
}
...
export async function POST(req: NextRequest, { params }: RouteParams): Promise<Response> {
  ...
  try {
    ...
    console.error("[api/chat/sources/promote] listener config missing:", error);
    return jsonError("Promotion is not configured", 500);
  }

  const { ledgerId: rawLedgerId } = await params;
  const ledgerIdResult = LEDGER_ID_SCHEMA.safeParse(rawLedgerId);
  if (!ledgerIdResult.success) {
    return jsonError("Invalid ledger id", 400);
  }
  const ledgerId = ledgerIdResult.data;
  ...
  try {
    upstream = await fetch(
      `${listenerConfig.url}/v1/chat/sources/${ledgerId}/promote`,
      {
        method: "POST",
        headers: {
    ...
    console.error("[api/chat/sources/promote] upstream fetch failed:", error);
  ...
  if (!upstream.ok) {
    if (
      ...
      console.error(`[api/chat/sources/promote] upstream ${upstream.status}:`, reason);
      return jsonError(
    ...
    console.error(`[api/chat/sources/promote] upstream ${upstream.status}:`, detail);
